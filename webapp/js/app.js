...
document.addEventListener("DOMContentLoaded", () => {
    ...
    loadHomeSlidingNotice(); // হোম স্ক্রিনের স্লাইডিং নোটিশ লোড করার ফাংশন কল করা হলো
...
/* =========================================
   Home Sliding Notice Integration
========================================= */
function loadHomeSlidingNotice() {
    const noticesRef = ref(db, 'webapp/notices');
    onValue(noticesRef, (snapshot) => {
        const data = snapshot.val();
        
        // হোম স্ক্রিনের মারকিউ বা স্লাইডিং টেক্সট কন্টেইনার (আপনার কোডের ক্লাস অনুযায়ী পরিবর্তন করে নিতে পারেন)
        const marqueeContainer = document.querySelector(".breaking-news-text"); 

        if (!data) return;

        let slidingNotices = Object.values(data).filter(n => n.type === 'sliding');

        if (slidingNotices.length > 0) {
            let latestNotice = slidingNotices[slidingNotices.length - 1];
            let isOff = localStorage.getItem("sliding_notice_disabled") === "true";
            
            if (!isOff && marqueeContainer) {
                marqueeContainer.innerText = `${latestNotice.title}: ${latestNotice.message}`;
            }
        }
    });
}
...
export function initNotificationSystem() {
    ...
    if (publishSlidingBtn) {
        publishSlidingBtn.addEventListener("click", async () => {
            ...
            const title = document.getElementById("slidingTitle")?.value.trim();
            const message = document.getElementById("slidingMessage")?.value.trim();
            ...
            try {
                ...
                showToast("✅ স্লাইডিং নোটিশ সফলভাবে প্রকাশিত হয়েছে!");
                ...
                showToast("❌ নোটিশ প্রকাশ করতে ব্যর্থ হয়েছে");
            }
        });
    }
    ...
    onValue(noticesRef, (snapshot) => {
        ...
        if (snapshot.exists()) {
            ...
            let keys = Object.keys(notices).reverse();
            keys.forEach(key => {
                const notice = notices[key];
                const dateStr = new Date(notice.time).toLocaleString('bn-BD');
                
                const itemDiv = document.createElement("div");
                itemDiv.style.cssText = "padding: 10px 0; border-bottom: 1px solid var(--border-color, #eee);";
                itemDiv.innerHTML = `
                    <h4 style="margin: 0 0 4px 0; font-size: 16px; color: var(--text-main);">${escapeHTML(notice.title)}</h4>
                    <p style="margin: 0 0 4px 0; font-size: 14px; color: var(--text-muted, #666);">${escapeHTML(notice.message)}</p>
                    <small style="font-size: 11px; color: var(--text-muted);">${dateStr}</small>
                `;
                listContainer.appendChild(itemDiv);
            });

            if (notifBadge && document.getElementById("notificationModal")?.classList.contains("hidden")) {
                notifBadge.classList.remove("hidden");
            }
        } else {
            listContainer.innerHTML = `<p style="text-align: center; color: var(--text-muted);">কোনো নোটিফিকেশন নেই</p>`;
        }
    });
}
...
function setupEvents() {
    ...
    document.getElementById("addSubCategoryBtn")?.addEventListener("click", () => openSubCategoryModal());
...
function openCategoryModal(editObj = null) {
    if (window.currentUserRole !== "admin") return;
    editingItem = editObj;
    ...
    if (editObj) {
        ...
        title.textContent = "নতুন Category";
        inputName.value = "";
...
function openSubCategoryModal(editObj = null) {
    if (window.currentUserRole !== "admin") return;
    editingItem = editObj;

    const title = document.getElementById("subCategoryModalTitle");
    const inputName = document.getElementById("subCategoryNameInput");

    if (editObj) {
        if (title) title.textContent = "Sub-Category এডিট করুন";
        if (inputName) inputName.value = editObj.name || "";
    } else {
        if (title) title.textContent = "নতুন Sub-Category";
...
async function saveSubCategory() {
    ...
    if (!name) return showToast("Sub-Category Name লিখুন");
    if (!currentCategoryId) return showToast("মূল ক্যাটাগরি সিলেক্ট করা নেই");
    ...
    refreshCurrentView();
    showToast("Sub-Category সেভ করা হয়েছে");
}
