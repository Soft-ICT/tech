...
function setupEvents() {
    ...
    // Admin Button Click & Long Press Handler
    const adminBtn = document.getElementById("adminLoginBtn");
    if (adminBtn) {
...
function renderRequestsList(requestsData) {
    ...
    if (reqKeys.length === 0) {
        container.innerHTML = `<div style="text-align:center; padding: 20px; color: var(--text-muted);">কোনো পেন্ডিং রিকোয়েস্ট নেই</div>`;
        return;
    }
    ...
    reqKeys.forEach(devId => {
        ...
        card.innerHTML = `
            <div>
                ...
                <div style="font-size: 13px; color: var(--text-muted);">💼 ${escapeHTML(req.designation || "পদবী নেই")}</div>
                <div style="font-size: 11px; color: #888; margin-top: 4px;">ID: ${devId}</div>
            </div>
            <div style="display: flex; gap: 6px;">
                <button class="btn-approve-req" style="background: #22c55e; color: white; border: none; padding: 6px 10px; border-radius: 5px; cursor: pointer; font-size: 13px;">Approve</button>
                <button class="btn-reject-req" style="background: #ef4444; color: white; border: none; padding: 6px 10px; border-radius: 5px; cursor: pointer; font-size: 13px;">Reject</button>
            </div>
        `;
...
function showDataPage(dataId) {
    ...
    // প্রশাসনিক তথ্য এবং স্থায়ী ঠিকানা—দুইটিই শুধু Admin এবং Verified Device-এর জন্য দৃশ্যমান থাকবে
