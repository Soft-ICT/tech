...
/* =========================================
   ...
   আগে app.js-এর একদম উপরে থাকা static import Firebase SDK
   নেটওয়ার্ক থেকে আনতে ব্যর্থ হলে পুরো app.js module-ই কখনো
   রান হতো না (ES module-এর নিয়ম — static import ব্যর্থ হলে
   পুরো ফাইল আটকে যায়)। এখন dynamic import() ব্যবহার করা
   হচ্ছে, try/catch দিয়ে ঘেরা — ব্যর্থ হলেও বাকি অ্যাপ (যেমন
   localStorage-এর cached ডাটা দেখানো) স্বাভাবিকভাবে চলবে।
========================================= */
...
async function ensureFirebase() {
    ...
    try {
        ...
        fbDb = firebaseModule.db;

        firebaseReady = true;
        return true;
    } catch (error) {
        console.error("Firebase SDK load failed (offline?):", error);
        return false;
    }
}
...
/* =========================================
   ...
   (Realtime listener — অনলাইনে অন্য কোনো ইউজার/ডিভাইস
   থেকে ডাটা পরিবর্তন হলে সাথে সাথে এই অ্যাপেও আপডেট হবে।

   এর পাশাপাশি প্রতিবার নতুন ডাটা এলে সেটা localStorage-এ
   সংরক্ষণ করা হয়। ফলে অফলাইনে অ্যাপ খুললে, Firebase থেকে
   রেসপন্স আসার আগ পর্যন্ত localStorage-এর সবশেষ কপি সাথে
   সাথে দেখানো যায় — নাহলে Firebase RTDB ওয়েব SDK নিজে থেকে
   ডাটা ডিস্কে সংরক্ষণ রাখে না, তাই অফলাইনে "0টি Category"
   দেখাতো।)
========================================= */

const DB_CACHE_KEY = "phonebook_offline_cache";

function saveDatabaseCache(data) {
    try {
        localStorage.setItem(DB_CACHE_KEY, JSON.stringify(data));
        showDebugBanner(
            "✅ CACHE SAVED — Categories: " +
            (data.categories ? data.categories.length : 0) +
            " | " + new Date().toLocaleTimeString()
        );
    } catch (error) {
        console.error("Offline cache save failed:", error);
        showDebugBanner("❌ CACHE SAVE FAILED: " + error.message);
    }
}
...
/* ==== সাময়িক ডিবাগ ব্যানার — সমস্যা ধরার পর মুছে ফেলা হবে ==== */
function showDebugBanner(text) {
    ...
    if (!banner) {
        ...
        banner.style.cssText =
            ...
            "background:#d90000;color:#fff;font-size:12px;line-height:1.4;" +
            "padding:6px 8px;text-align:center;font-family:monospace;" +
            "box-shadow:0 2px 6px rgba(0,0,0,.4);word-break:break-all;";
...
async function loadDatabase() {
    if (databaseListenerAttached) {
        // listener আগে থেকেই চালু আছে (একাধিকবার login/logout
        // হলেও একাধিক listener যুক্ত হওয়া ঠেকানোর জন্য)
        return;
    }

    databaseListenerAttached = true;

    // Firebase থেকে রেসপন্স আসার আগেই, localStorage-এ থাকা
    // সবশেষ ডাটা সাথে সাথে দেখিয়ে দেওয়া হচ্ছে (বিশেষ করে অফলাইনে
    // এটাই একমাত্র ডাটা যা পাওয়া যাবে)
    const cachedData = loadDatabaseCache();

    showDebugBanner(
        cachedData
            ? "🔍 CACHE FOUND on load — Categories: " + (cachedData.categories ? cachedData.categories.length : 0)
            : "🔍 NO CACHE FOUND on load (localStorage empty)"
    );
    ...
    if (!ok) {
        showDebugBanner(
            "⚠️ FIREBASE SDK লোড ব্যর্থ (অফলাইন?) — cached ডাটা দেখানো হচ্ছে"
        );
        if (!cachedData) {
            showToast("ডাটা লোড করতে সমস্যা হয়েছে — ইন্টারনেট চেক করুন");
        }
        return;
    }

    fbOnValue(
        fbRef(fbDb, "webapp/public_data"),
        function (snapshot) {
            if (snapshot.exists()) {
            ...
            saveDatabaseCache(database);
            refreshCurrentView();
        },
        function (error) {
            console.error("Database load error:", error);
            showDebugBanner("❌ FIREBASE onValue ERROR: " + error.message);
            if (!cachedData) {
...
async function saveDatabase() {
    ...
    const ok = await ensureFirebase();
    if (!ok) {
        showToast("অফলাইনে আছেন — ইন্টারনেট সংযোগ ছাড়া সেভ করা যাবে না");
        return;
    }

    try {
        await fbSet(fbRef(fbDb, "webapp/public_data"), database);
    } catch (error) {
