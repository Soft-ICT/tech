...
function renderCategories(searchVal = "") {
    ...
    categoriesToShow.forEach(category => {
        ...
        const imageHtml = category.image
            ? `<img src="${escapeHTML(category.image)}" alt="${escapeHTML(category.name)}" class="cat-card-img" onerror="this.style.display='none'">`
            : "";
...
function showCategoryView(id) {
    ...
    // ১. টপবারের টাইটেল আপডেট করা
    const appTitle = document.getElementById("appTitle");
    if (appTitle) {
        appTitle.textContent = category.name;
    }

    // ২. নেভিগেশন স্ট্যাটাস আপডেট
    setNavState(true);

    // ৩. সাব-টুলবার হাইড করা
    ...
    // ৪. পেজের বডির ভেতরের ডুপ্লিকেট টাইটেল এলিমেন্ট হাইড করা
...
function showMainDashboardView(updateHistory = true) {
    ...
    // ১. মূল পেজে ফিরলে টাইটেল "Police Phonebook" সেট করা
    const appTitle = document.getElementById("appTitle");
    if (appTitle) {
        appTitle.textContent = "Police Phonebook";
    }

    // ২. মেনু আইকন সক্রিয় করা
    setNavState(false);
    ...
    if (subCategories.length > 0) {
        ...
        subCategories.forEach(sub => {
            ...
            const imageHtml = sub.image
                ? `<img src="${escapeHTML(sub.image)}" alt="${escapeHTML(sub.name)}" class="cat-card-img" onerror="this.style.display='none'">`
                : "";
