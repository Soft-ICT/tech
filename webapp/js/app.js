...
/* =========================================
   Authentication & Requests Monitoring
========================================= */
watchAuth((user, role) => {
    ...
    if (!user) {
        ...
        if (adminBtn) {
            adminBtn.innerHTML = `🟢 Admin <span id="reqCountBadge" class="req-badge hidden">0</span>`;
...
function toggleTheme() {
    ...
    showToast(newTheme === 'dark' ? "নাইট মোড অন করা হয়েছে" : "ডে মোড অন করা হয়েছে");
}
...
function setupEvents() {
    ...
    // Admin Button Click & Long Press Handler Setup
    const adminBtn = document.getElementById("adminLoginBtn");
    if (adminBtn) {
        setupAdminButtonLongPress(adminBtn);
    }
...
/* =========================================
   Admin Button Long Press & Click Logic
========================================= */
function setupAdminButtonLongPress(btnElement) {
    let pressTimer = null;
    ...
    const startPress = () => {
        ...
        pressTimer = setTimeout(() => {
            ...
            if (window.currentUserRole === "admin") {
                ...
                showToast("শুধুমাত্র অ্যাডমিন ভেরিফিকেশন রিকোয়েস্ট দেখতে পাবেন");
            }
        }, 800);
    };
    ...
    btnElement.addEventListener("mouseup", cancelPress);
    btnElement.addEventListener("mouseleave", cancelPress);
    btnElement.addEventListener("touchend", cancelPress);
...
/* =========================================
   Admin Verification Requests Handling
========================================= */
...
function renderRequestsList(requestsData) {
    const container = document.getElementById("requestsListContainer");
    const badge = document.getElementById("reqCountBadge");
    if (!container) return;

    const reqKeys = Object.keys(requestsData);

    if (badge) {
        if (reqKeys.length > 0) {
            badge.textContent = reqKeys.length;
    ...
    if (reqKeys.length === 0) {
        container.innerHTML = `<div style="text-align:center; padding: 20px; color: var(--text-muted, #666);">কোনো পেন্ডিং রিকোয়েস্ট নেই</div>`;
        return;
    }

    reqKeys.forEach(devId => {
        const req = requestsData[devId];
        const card = document.createElement("div");
        card.style.cssText = "background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); padding: 12px; margin-bottom: 10px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center;";

        card.innerHTML = `
            <div>
                <div style="font-weight: bold; font-size: 15px;">👤 ${escapeHTML(req.name || "অজ্ঞাত")}</div>
                <div style="font-size: 13px; color: #888;">💼 ${escapeHTML(req.designation || "পদবী নেই")}</div>
                <div style="font-size: 11px; color: #aaa; margin-top: 4px;">ID: ${devId}</div>
            </div>
            <div style="display: flex; gap: 6px;">
                <button class="btn-approve-req primary-btn" style="background: #22c55e; border:none; padding: 6px 10px; font-size: 12px;">Approve</button>
                <button class="btn-reject-req danger-btn" style="padding: 6px 10px; font-size: 12px;">Reject</button>
            </div>
        `;

        card.querySelector(".btn-approve-req").addEventListener("click", () => approveDevice(devId));
        card.querySelector(".btn-reject-req").addEventListener("click", () => rejectDevice(devId));

        container.appendChild(card);
    });
}

async function approveDevice(devId) {
    try {
        ...
        showToast("✅ ডিভাইসটি সফলভাবে অ্যাপ্রুভ করা হয়েছে!");
    } catch (e) {
        showToast("অ্যাপ্রুভ করতে সমস্যা হয়েছে");
    }
}

async function rejectDevice(devId) {
    const isConfirmed = await customConfirm("আপনি কি নিশ্চিত এই রিকোয়েস্টটি বাতিল করতে চান?", "বাতিল নিশ্চিতকরণ", "হ্যাঁ, বাতিল করুন");
    if (!isConfirmed) return;

    try {
        await remove(ref(db, `webapp/verification_requests/${devId}`));
        showToast("❌ রিকোয়েস্ট বাতিল করা হয়েছে");
    } catch (e) {
...
function openCategoryModal(isSubCategory = false, editObj = null) {
    ...
    if (editObj) {
        if (title) title.textContent = "Category এডিট করুন";
        if (inputName) inputName.value = editObj.name || "";
...
function showCategoryView(id) {
    ...
    if (detailsTitle) detailsTitle.textContent = category.name;

    setNavState(true);

    const subToolbar = document.getElementById("allSearchBtn");
    if (subToolbar) {
...
function showDataPage(dataId) {
    ...
    container.innerHTML = `
        ...
        <div class="quick-action-grid" style="display:flex; gap:10px; margin: 15px 0;">
            <button id="btnMobileCall" class="primary-btn" style="flex:1;">📱 মোবাইল</button>
            <button id="btnPhoneCall" class="secondary-btn" style="flex:1;">☎️ টেলিফোন</button>
            <a href="${item.email ? "mailto:" + item.email : "#"}" class="secondary-btn" style="flex:1; text-align:center; text-decoration:none;">✉️ ইমেইল</a>
            <button id="btnShareContact" class="primary-btn" style="flex:1;">🔗 শেয়ার</button>
        </div>
        ...
        <div class="details-info-list">
            ...
            ${showVerifyBtnInDetails ? `
                <div style="text-align: right; margin-top: 10px;">
                    <button class="primary-btn" id="detailsVerifyBtn" title="প্রশাসনিক ডাটা ও স্থায়ী ঠিকানা দেখার জন্য ১০ সে. চেপে রাখুন">Get Verify</button>
                </div>` : ""
            }
        </div>
    `;
