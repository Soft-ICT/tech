...
function initNotificationSystem() {
    ...
    onValue(noticesRef, (snapshot) => {
        if (snapshot.exists()) {
            console.log("Notices loaded:", snapshot.val());
        }
    });
}
...
// শুধুমাত্র ডাটার জন্য স্মার্ট সর্টিং (সংখ্যা প্রথমে, নাম বর্ণানুক্রমিকভাবে)
function sortItemsBySmartCriteria(items) {
...
function renderAllSearch() {
    ...
    allData = sortItemsBySmartCriteria(allData);
...
function renderCategories(searchVal = "") {
    ...
    // ক্যাটাগরি যেভাবে সেভ করা হয়েছে বা ডিফল্ট অর্ডারে থাকবে (স্মার্ট সর্ট তুলে দেওয়া হলো)
    list.innerHTML = "";
...
async function saveData() {
    ...
    // শর্ত ২: নাম এবং মোবাইল নাম্বার বাধ্যতামূলক
    if (!name) return showToast("নাম প্রদান করুন");
    if (!mobile) return showToast("মোবাইল নম্বর প্রদান করুন");
...
function renderCategoryDetails(searchVal = "") {
    ...
    // সাব-ক্যাটাগরি আগের নিয়মে থাকবে (স্মার্ট সর্ট বাদ দেওয়া হয়েছে)
    ...
    noHeaderData = sortItemsBySmartCriteria(noHeaderData);
    ...
    headers.forEach(header => {
        ...
        // শুধুমাত্র হেডারের অভ্যন্তরের ডাটাগুলো স্মার্ট সর্ট হবে
        let matchedData = sortItemsBySmartCriteria([...headerAllData]);
        ...
        if (!filterText || isHeaderMatched || matchedData.length > 0) {
            const displayData = isHeaderMatched ? sortItemsBySmartCriteria([...headerAllData]) : matchedData;
...
function renderDataDetailsContent(item) {
    ...
    container.innerHTML = `
        ...
        <div class="quick-action-grid">
            ...
            <a href="#" id="btnEmailAction" class="action-btn-round btn-email-round">✉️ ইমেইল</a>
    ...
    // শর্ত ৩: ইমেইল না থাকলে টোস্ট মেসেজ শো করবে
    const emailBtn = document.getElementById("btnEmailAction");
    if (emailBtn) {
        emailBtn.addEventListener("click", (e) => {
            e.preventDefault();
            if (!item.email || item.email === "ইমেইল নেই") {
                showToast("⚠️ ইমেইল নম্বর নেই!");
            } else {
...
// শর্ত ৩: ডায়াল বা হোয়াটসঅ্যাপ নম্বর না থাকলে টোস্ট মেসেজ শো করার লজিক
function setupSmartCallAndWhatsApp(element, rawNumber, typeName) {
    if (!element) return;

    if (!rawNumber || rawNumber === "মোবাইল নেই" || rawNumber === "টেলিফোন নেই" || rawNumber.trim() === "") {
        element.style.opacity = "0.7";
        element.addEventListener('click', (e) => {
            e.preventDefault();
            showToast(`⚠️ কোনো ${typeName} নম্বর নেই!`);
        });
        return;
    }
    ...
    const startPress = () => {
        ...
        pressTimer = setTimeout(() => {
            ...
            if (cleanNumber) {
                ...
                showToast("⚠️ হোয়াটসঅ্যাপ নম্বর পাওয়া যায়নি!");
            }
        }, 600);
    };
    ...
    element.addEventListener('click', (e) => {
        ...
        if (!isLongPress) {
            ...
            if (cleanNumber) {
                ...
                showToast(`⚠️ ডায়াল করার মতো কোনো ${typeName} নম্বর নেই!`);
            }
        }
        isLongPress = false;
    });
