...
/* =========================================
   IndexedDB Database Load & Cache (Updated)
========================================= */
function loadLocalCache() {
    const request = indexedDB.open("PolicePhonebookDB", 1);
    ...
    request.onupgradeneeded = (event) => {
        ...
        if (!dbInstance.objectStoreNames.contains("store")) {
            dbInstance.objectStoreNames.contains("store") || dbInstance.createObjectStore("store");
        }
    };

    request.onsuccess = (event) => {
        const dbInstance = event.target.result;
        if (!dbInstance.objectStoreNames.contains("store")) return;
        
        const transaction = dbInstance.transaction(["store"], "readonly");
        const store = transaction.objectStore("store");
        const getRequest = store.get("police_phonebook_data");
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            request.onsuccess = (event) => {
                ...
                const store = transaction.objectStore("store");
                store.put(database, "police_phonebook_data");
            };
...
/* =========================================
   IndexedDB Database Save (Updated)
========================================= */
