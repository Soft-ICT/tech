...
function setupEvents() {
    ...
    document.getElementById("deviceRequestsBtn")?.addEventListener("click", openRequestsModal);
...
/* =========================================
   Verification Requests (User & Admin)
========================================= */
...
async function openRequestsModal() {
    ...
    try {
        const snapshot = await get(ref(db, "webapp/verification_requests"));
        container.innerHTML = "";

        if (!snapshot.exists()) {
            container.innerHTML = `<div style="text-align:center; padding: 20px; color: var(--text-muted);">কোনো নতুন রিকোয়েস্ট নেই</div>`;
            return;
        }

        const data = snapshot.val();
        Object.keys(data).forEach(devId => {
            const req = data[devId];
            const div = document.createElement("div");
            div.className = "details-info-box";
            div.style.marginBottom = "10px";
            div.innerHTML = `
                <div><strong>${escapeHTML(req.name)}</strong> (${escapeHTML(req.designation)})</div>
                <div style="font-size: 11px; color: var(--text-muted); margin-top:2px;">ID: ${devId}</div>
                <div style="display: flex; gap: 8px; margin-top: 8px;">
                    <button class="primary-btn btn-approve-dev" style="padding: 4px 10px; font-size: 12px;">Approve</button>
                    <button class="danger-btn btn-reject-dev" style="padding: 4px 10px; font-size: 12px;">Reject</button>
                </div>
            `;

            div.querySelector(".btn-approve-dev").addEventListener("click", async () => {
                await set(ref(db, `webapp/approved_devices/${devId}`), { status: "approved", approvedAt: Date.now() });
                await remove(ref(db, `webapp/verification_requests/${devId}`));
                showToast("অ্যাপ্রুভ করা হয়েছে!");
                openRequestsModal();
            });

            div.querySelector(".btn-reject-dev").addEventListener("click", async () => {
                await remove(ref(db, `webapp/verification_requests/${devId}`));
                showToast("বাতিল করা হয়েছে");
                openRequestsModal();
            });
        ...
        container.innerHTML = `<div style="text-align:center; padding: 20px; color: #ef4444;">রিকোয়েস্ট লোড হতে সমস্যা হয়েছে</div>`;
    }
}
...
function createDataCardElement(item) {
    ...
    const verifyBtnHtml = (!isDeviceVerified && !isAdmin)
        ? `<button class="btn-get-verify" title="প্রশাসনিক ডাটা ভেরিফিকেশনের জন্য ১০ সে. চেপে রাখুন">Get Verify</button>`
        : "";
    ...
    const verifyBtn = dataEl.querySelector(".btn-get-verify");
    if (verifyBtn) {
        verifyBtn.addEventListener("click", e => e.stopPropagation());
        setupHoldToVerify(verifyBtn);
    }
        ...
        <div class="details-info-list">
            ...
            ${showVerifyBtnInDetails ? `
                <div style="text-align: right; margin-top: 5px;">
                    <button class="btn-get-verify" id="detailsVerifyBtn">Get Verify</button>
                </div>` : ""
            }
        </div>
    `;
