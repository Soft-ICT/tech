/* =========================================
   Notification System Module (Fixed & Merged)
========================================= */
...
function createNotificationUI() {
    ...
    <div id="notificationModal" class="modal hidden" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.6); z-index: 99999; align-items: center; justify-content: center;">
        <div class="modal-content notification-modal-content" style="max-width: 680px; width: 95%; background: var(--bg-card, #fff); border-radius: 8px; overflow: hidden; box-shadow: 0 5px 25px rgba(0,0,0,0.4);">
            <div class="modal-header" style="padding: 15px; border-bottom: 1px solid var(--border-color, #ddd); display: flex; justify-content: space-between; align-items: center; background: var(--bg-header, #f8f9fa);">
                <h2 style="margin:0; font-size: 18px; color: var(--text-main, #333);">📢 নোটিফিকেশন ম্যানেজমেন্ট প্যানেল</h2>
                <button class="close-btn" data-close="notificationModal" type="button" style="background:none; border:none; font-size: 20px; cursor:pointer; color: var(--text-main, #666);">✕</button>
            </div>
            ...
            <div class="modal-actions" style="padding: 10px 15px; border-top: 1px solid var(--border-color); display: flex; justify-content: flex-end; gap: 10px; background: var(--bg-header, #f8f9fa);">
...
function setupNotificationEvents() {
    // সঠিকভাবে মোডাল ওপেন করার ইভেন্ট ডেলিগেশন
...
function listenToFirebaseNotices() {
    ...
    onValue(noticesRef, (snapshot) => {
        ...
        if (data) {
            ...
            if (activeHome) localStorage.setItem("active_home_notice", JSON.stringify(activeHome));
        } else {
