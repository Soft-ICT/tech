...
function applyBackground() {
  ...
  if (image) {
    ...
    /* 
     * IMPORTANT: 
     * Image fixed থাকবে এবং স্ক্রল করলে উঠানামা করবে না 
     */
    document.body.style.setProperty(
...
function isColorFormatterElement(el) {
  ...
  if (
    ...
    className.includes("formatted") ||
    className.includes("gradient") ||
    className.includes("rainbow") ||
...
/* ============================================================
   GALLERY UI 
   কোনো লেখা থাকবে না। শুধু Image icon.
   ============================================================ */
function createBackgroundImageUI() {
  /* 
   * Already created 
   */
  if (document.getElementById("customBackgroundImageBox")) {
    return;
  }
  ...
  const parent = bgColorInput.parentElement;
  if (!parent) {
    return;
  }
  ...
  chooseButton.title = "Background Image";
  chooseButton.setAttribute("aria-label", "Background Image");
  chooseButton.style.width = "42px";
  chooseButton.style.height = "42px";
  chooseButton.style.padding = "0";
  chooseButton.style.fontSize = "20px";
  chooseButton.style.display = "flex";
  chooseButton.style.alignItems = "center";
  chooseButton.style.justifyContent = "center";
  ...
  removeButton.setAttribute("aria-label", "Remove Background Image");
  ...
  reader.onload = function (event) {
    ...
    image.onload = function () {
      ...
      try {
        ...
        console.warn("Background image save failed:", error);
        alert("ছবিটি সংরক্ষণ করা যায়নি। ছোট ছবি নির্বাচন করুন।");
      }
    };
...
function loadGoogleTranslate() {
   ...
   */
  let box = document.getElementById("google_translate_element");
  if (!box) {
  ...
  window.googleTranslateElementInit = function () {
    try {
      if (window.google && window.google.translate) {
        ...
        setTimeout(applyGoogleLanguage, 500);
      }
    } catch (error) {
...
function applyGoogleLanguage() {
  const language = localStorage.getItem(CS_LANGUAGE) || "bn";
  ...
  const target = language === "en" ? "en" : "bn";
...
function applyLanguage(language) {
  ...
  if (language === "en") {
    loadGoogleTranslate();
    setTimeout(applyGoogleLanguage, 1000);
  } else {
    /* 
     * বাংলা 
     */
    if (googleTranslateReady) {
      setTimeout(applyGoogleLanguage, 300);
...
export function setupCustomSettingsListener() {
  ...
  document.addEventListener("click", function (e) {
    if (e.target && e.target.id === "chooseBgImageBtn") {
      openGallery();
    }
    if (e.target && e.target.id === "removeBgImageBtn") {
